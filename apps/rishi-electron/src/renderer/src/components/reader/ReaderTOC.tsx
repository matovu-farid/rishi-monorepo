...
export function ReaderTOC({
  ...
  const [activeTab, setActiveTab] = React.useState<'contents' | 'bookmarks'>('contents')
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
        <SheetHeader className={cn('p-4 border-b sticky top-0 z-10 border-gray-200 bg-white')}>
        ...
        {activeTab === 'contents' ? (
          <div className="overflow-y-auto h-[calc(100vh-73px)]">{tocContent}</div>
        ) : (
          <BookmarksList bookSyncId={bookSyncId} onNavigate={onBookmarkNavigate} />
        )}
      </SheetContent>
    </Sheet>
  )
