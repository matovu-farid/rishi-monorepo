...
const mockDecryptString = vi.fn((buf: Buffer) => buf.toString().replace('enc:', ''))
...
describe('auth IPC handlers', () => {
  ...
  describe('auth:refreshToken', () => {
    ...
    it('refreshes token when expiry is within 1 day', async () => {
      ...
      mockFetch.mockResolvedValueOnce(jsonResponse({ token: 'new-token', expiresAt: newExpiry }))
  ...
  describe('auth:getUser', () => {
    ...
    it('throws on invalid userId format', async () => {
      const handler = getHandler('auth:getUser')
      await expect(handler({}, '../../../etc/passwd')).rejects.toThrow('Invalid userId format')
      expect(mockFetch).not.toHaveBeenCalled()
    })
    ...
    it('throws on 401 response', async () => {
      ...
      const handler = getHandler('auth:getUser')
      await expect(handler({}, 'user_123')).rejects.toThrow('Session expired')
    })
  ...
  describe('auth:getDebug', () => {
    it('fetches debug entries from worker', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('/api/auth/debug/test-state'))
    })
    ...
    it('throws when fetch fails', async () => {
      ...
      const handler = getHandler('auth:getDebug')
      await expect(handler({}, 'test-state')).rejects.toThrow('Failed to get auth debug')
    })
  })
})
