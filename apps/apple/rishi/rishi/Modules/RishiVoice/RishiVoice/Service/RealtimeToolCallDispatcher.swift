...
/// into the SDK's `RealtimeConversation`. The adapter owns the `RealtimeConversation`; the
...
struct RealtimeToolCallDispatcher: Sendable {
    ...
    /// at `~/Library/.../swift-realtime-openai/Sources/UI/RealtimeConversation.swift`).
    ...
    /// supplied `RealtimeConversation`. Throws a `RealtimeClientError` on send failure.
    ///
    /// The `RealtimeConversation` is `@MainActor`-isolated, so the send hops to main.
    func sendResult(
        callId: String,
        payload: String,
        on conversation: RealtimeConversation
    ) async throws {
