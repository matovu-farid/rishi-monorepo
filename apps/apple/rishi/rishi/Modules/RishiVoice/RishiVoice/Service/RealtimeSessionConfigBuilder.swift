...
struct RealtimeSessionConfigBuilder: Sendable {
    ...
    static func makePCM24kFormat() -> RealtimeSession.AudioFormat {
        let data = try! JSONSerialization.data(withJSONObject: [
            "rate": 24000,
            "type": "audio/pcm",
        ])
        return try! JSONDecoder().decode(RealtimeSession.AudioFormat.self, from: data)
    }
    ...
    /// hands-free.
    static var inputNoiseReduction: RealtimeSession.Audio.Input.NoiseReduction {
    ...
    func makeSessionAudio(language: String? = "en") -> RealtimeSession.Audio {
        let pcm24k = Self.makePCM24kFormat()
        let input = RealtimeSession.Audio.Input(
            format: pcm24k,
        ...
        let output = RealtimeSession.Audio.Output(
            voice: .alloy,
            speed: 1.0,
            format: pcm24k
        )
        return RealtimeSession.Audio(input: input, output: output)
    }
    ...
    func configure(
        ...
        session.toolChoice = Tool.Choice.auto
    }
