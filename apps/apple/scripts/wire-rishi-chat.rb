#!/usr/bin/env ruby
# Wire the RishiChat local SwiftPM package into rishi.xcodeproj.
# Idempotent: safe to re-run.
#
# Mirrors apps/apple/scripts/wire-rishi-audio.rb (Phase 8 Plan 08-01) and
# apps/apple/scripts/wire-rishi-reader.rb (Phase 5 Plan 05-01).
...
if ref.nil?
  ...
  puts "[wire-rishi-chat] Added local package reference #{PKG_PATH}"
else
  puts "[wire-rishi-chat] Local package reference #{PKG_PATH} already present"
end
...
if product_dep.nil?
  ...
  puts "[wire-rishi-chat] Added product dependency #{PKG_NAME} to rishi target"
else
  puts "[wire-rishi-chat] Product dependency #{PKG_NAME} already on rishi target"
end
...
unless already_linked
  ...
  puts "[wire-rishi-chat] Linked #{PKG_NAME} into rishi frameworks build phase"
else
  puts "[wire-rishi-chat] #{PKG_NAME} already in rishi frameworks build phase"
end

project.save
puts "[wire-rishi-chat] Saved #{PROJ_PATH}"
