...
export const TOOLS = [
  ...
  { name: "read_app_logs", description: "Read the most recent redacted DEBUG logs from one already-running Apple app target.", inputSchema: { type: "object", properties: { app: { type: "string", enum: ["catalyst", "iphone17"] }, limit: { type: "integer", minimum: 1, maximum: 500 } }, required: ["app"], additionalProperties: false } },
  ...
  { name: "click_text", description: "Press one visible button or menu item by its exact accessible text without launching another app.", inputSchema: { type: "object", properties: { app: { type: "string", enum: ["catalyst", "iphone17"] }, text: { type: "string", minLength: 1 }, index: { type: "integer", minimum: 0 } }, required: ["app", "text"], additionalProperties: false } },
