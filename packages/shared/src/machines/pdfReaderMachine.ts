/**
 * PDF reader XState machine. Ported verbatim from
 * `apps/rishi-electron/src/renderer/src/machines/pdfReaderMachine.ts`.
 *
 * Encodes the seek lockout + debounced persistence pattern that prevents
 * scroll-position races during initial restore and TOC/thumbnail jumps.
 * The save actor is injected so platforms can supply their own IPC layer
 * (electron uses window.electron, mobile passes a Drizzle/MMKV writer).
 */
