...
async function signInFreshUser(env: Env): Promise<SignInResult> {
  ...
  const data = (await res.json()) as { token: string; userId: string; email: string };
  return data;
}

async function deleteUser(env: Env, email: string): Promise<void> {
 ...
 */
async function mintPortalUrl(token: string): Promise<string> {
  ...
  const data = (await res.json()) as Partial<PortalResult>;
  if (!data.url) {
 ...
 */
async function probePortalUrl(url: string): Promise<{ status: number; host: string | null }> {
