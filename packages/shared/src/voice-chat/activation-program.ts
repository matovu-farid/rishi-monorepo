...
import type { RealtimeUsageInput } from '../billing/realtime-usage-accumulator'
 ...
 */
type CaptureFn = (err: unknown, context: { operation: string; step: string }) => void
...
export function makeActivationProgram(a: ActivationDeps): ActivationProgram {
  ...
  const capture: CaptureFn = deps.captureError ?? (() => undefined)
  ...
  const activationPipeline = (
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        if (buffered) {
          yield* Effect.promise(() => replayBufferedSpeech(buffered, built.session, deps.ipc, capture))
        }
