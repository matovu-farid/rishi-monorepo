...
export function registerFsHandlers(): void {
  ipcMain.handle(
    "fs:checkFileSize",
    async (_event, filePath: string, format: string) => {
      try {
  ...
  ipcMain.handle(
    "fs:unzip",
    async (_event, filePath: string, outDir: string) => {
      try {
        ...
        for (const [relativePath, file] of Object.entries(zip.files)) {
          ...
          if (!targetPath.startsWith(resolvedOutDir + path.sep) && targetPath !== resolvedOutDir) {
            throw new Error(
              `Zip entry "${relativePath}" would escape output directory (Zip Slip)`
            );
          }
        ...
        throw new Error(
          `Failed to unzip: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    }
  );
