/**
 * Port of `apps/rishi-electron/src/renderer/src/machines/navigationHistory/navigationHistoryMachine.test.ts`
 * for the shared machine (T-P1.6 / NAVHIST-001, closes R-007).
 *
 * Differences from electron:
 *   - `resumeMap` is a `Record<string, AnchorPoint>` (NOT `Map`) per
 *     REVIEW-01 MINOR-05 — JSON-serializable for XState devtools.
 *     All `resumeMap.get(k)` / `.size` / `.keys()` reads in the electron
 *     tests are rewritten to plain object indexing.
 *   - Adds a JSON-roundtrip test to lock in the serialization contract.
 *
 * Fails today because the shared machine does not yet exist.
 */
import { describe, it, expect, beforeEach, afterEach, vi } from 'vitest'
import { createActor } from 'xstate'
import { navigationHistoryMachine } from './navigationHistoryMachine'
import type { AnchorPoint, PositionDescriptor } from './types'
...
describe('navigationHistoryMachine — lifecycle', () => {
  ...
  it('BOOK_CLOSED clears stack, resumeMap, currentPage and returns to inactive', () => {
    ...
    // resumeMap is a plain Record (NOT Map) — empty means no keys
    expect(Object.keys(snap.context.resumeMap)).toHaveLength(0)
...
describe('navigationHistoryMachine — resume map + pill', () => {
  ...
  it('entering engaged writes currentPage anchor into resumeMap[pageKey]', () => {
    ...
    actor.send({ type: 'ENGAGEMENT_TAP' })
    const snap = actor.getSnapshot()
    // resumeMap is a Record; index lookup replaces electron's `.get(k)`
    const anchor = snap.context.resumeMap['pdf:7']
  ...
  it('POP_BACK emits RESUME_REQUESTED with the popped anchor', () => {
    // Contract: pill click → POP_BACK → reader subscribers need
    // RESUME_REQUESTED with the popped anchor so they can seek back.
    const actor = startActor()
...
describe('navigationHistoryMachine — JSON serialization (T-P1.6 MINOR-05)', () => {
  beforeEach(() => vi.useFakeTimers())
  afterEach(() => vi.useRealTimers())

  const pos = (page: number, offset = 0): PositionDescriptor => ({ kind: 'pdf', page, offset })

  it('context survives JSON.stringify → JSON.parse round-trip with resumeMap entries intact', () => {
    // The shared machine MUST use `Record<string, AnchorPoint>` for
    // resumeMap (not `Map`) so XState devtools/inspectors can serialize
    // the snapshot. This test exercises the round-trip end to end.
    const actor = startActor()
    actor.send({ type: 'BOOK_OPENED', bookId: 'b', initialPosition: pos(7, 250) })
    actor.send({ type: 'PAGE_VISITED', position: pos(7, 250), ttsContext: { paragraphIndex: 3 } })
    actor.send({ type: 'ENGAGEMENT_TAP' })
    ...
    const snap = actor.getSnapshot()
    // Critical: this would silently lose entries if `resumeMap` were a Map
    // (Map serializes to `{}` via JSON.stringify and never round-trips).
    const round = JSON.parse(JSON.stringify(snap.context)) as typeof snap.context

    expect(round.bookId).toBe('b')
    expect(round.stack).toHaveLength(1)
    expect(round.resumeMap['pdf:7']).toBeDefined()
    expect(round.resumeMap['pdf:7'].position).toEqual(pos(7, 250))
    expect(round.resumeMap['pdf:7'].tts).toEqual({ paragraphIndex: 3 })
  })

  it('resumeMap is a plain object (not a Map instance)', () => {
    const actor = startActor()
    actor.send({ type: 'BOOK_OPENED', bookId: 'b', initialPosition: pos(1) })
    const ctx = actor.getSnapshot().context
    expect(ctx.resumeMap).not.toBeInstanceOf(Map)
    expect(Object.getPrototypeOf(ctx.resumeMap)).toBe(Object.prototype)
  })
})
