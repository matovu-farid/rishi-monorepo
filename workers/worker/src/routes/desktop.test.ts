...
const setSessionFor = (
  cookie: string,
  s: { user: { id: string }; session: { token: string } }
) => {
...
describe("POST /desktop/start/complete", () => {
  it("happy path: state exists -> 200, writes result, deletes state, writes completed sentinel", async () => {
    ...
    const result = redisStore.get(`desktop:result:${state}`) as
      | { session_token: string; user_id: string }
      | undefined
    ...
    const sentinel = redisStore.get(`desktop:completed:${state}`) as
      | { user_id: string; createdAt: number }
      | undefined
  ...
  it("second call by DIFFERENT user after state was consumed -> 400 state_expired", async () => {
    ...
    const body = (await attacker.json()) as { error: string }
