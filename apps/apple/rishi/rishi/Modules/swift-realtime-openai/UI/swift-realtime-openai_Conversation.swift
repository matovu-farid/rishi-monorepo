...
public final class RealtimeConversation: @unchecked Sendable {
	public typealias SessionUpdateCallback = (inout RealtimeSession) -> Void

	private let client: WebRTCConnector
	private var task: Task<Void, Error>!
	private let sessionUpdateCallback: SessionUpdateCallback?
	private let errorStream: AsyncStream<ServerError>.Continuation
	private let sessionStream: AsyncStream<RealtimeSession>.Continuation
	...
	/// RealtimeSession snapshots emitted whenever the server creates or updates the
	...
	/// consumers.
	public let sessionUpdates: AsyncStream<RealtimeSession>

	/// The current session for this conversation.
	public private(set) var session: RealtimeSession?
	...
	public required init(debug: Bool = false, configuring sessionUpdateCallback: SessionUpdateCallback? = nil) {
		...
		(sessionUpdates, sessionStream) = AsyncStream.makeStream(of: RealtimeSession.self)
	...
	/// Explicitly tear down the conversation. Cancels the event-handling task,
	/// closes the WebRTC peer (stops mic + playout and finishes the events
	/// stream), and finishes the error stream. REQUIRED before dropping the last
	/// reference: the detached `task` strongly retains `self` while awaiting
	/// `client.events`, which only finishes when the peer closes — a retain cycle
	/// that otherwise leaks the peer (it keeps streaming audio). Idempotent.
	...
	public func updateSession(withChanges callback: (inout RealtimeSession) throws -> Void) throws {
	...
	public func setSession(_ session: RealtimeSession) throws {
	...
	/// > Note: The `RealtimeConversation` class can automatically handle listening to the user's mic and playing back model responses.
...
private extension RealtimeConversation {
