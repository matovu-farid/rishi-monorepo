  ...
  it("should import a PDF via IPC and show it in library", () => {
    ...
    cy.window().then(async (win: any) => {
      ...
      try {
        ...
        cy.log("PDF metadata: " + JSON.stringify({ title: bookData?.title, kind: bookData?.kind }));
      } catch (err: any) {
        cy.log("getPdfData failed: " + err.message);
        // Fallback - create minimal book data
        bookData = { kind: "pdf", cover: [], title: "Test PDF", author: "Test", publisher: "", filepath: destPath, location: "", coverKind: null, version: 1 };
      }
