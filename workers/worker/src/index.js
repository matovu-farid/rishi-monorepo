...
function coerceLanguage(raw) {
    if (!raw)
        return "en";
    return ALLOWED_REALTIME_LANGUAGES.includes(raw)
        ? raw
        : "en";
}
...
function getOpenAI(apiKey) {
    if (_openai && _openaiApiKey === apiKey)
        return _openai;
...
let _speechOpenAI = null;
let _speechOpenAIKey = null;
function getSpeechOpenAI(apiKey) {
    if (_speechOpenAI && _speechOpenAIKey === apiKey)
...
function resolveElevenLabsModelId(model) {
...
export function buildRealtimeClientSecretsBody(input) {
 ...
 */
async function ttsCacheKey(text, voice, speed) {
...
async function elevenLabsTtsCacheKey(text, voice, model, speed) {
...
function resolveTtsResponseMode(value) {
    return value === "events" ? "events" : "raw";
}
function splitTtsBytes(bytes, chunkSize = TTS_EVENT_CHUNK_BYTES) {
    const chunks = [];
...
function encodeSseFrame(fields) {
    return `${fields.map(([key, value]) => `${key}: ${value}`).join("\n")}\n\n`;
}
function ttsChunkId(requestKey, index) {
    return `${requestKey}#${index.toString().padStart(8, "0")}`;
}
function ttsDoneId(requestKey) {
    return `${requestKey}#done`;
}
function buildTtsRawResponse(bytes, cacheStatus) {
...
function buildTtsEventResponse(bytes, requestKey, cacheStatus) {
    ...
    const stream = new ReadableStream({
        start(controller) {
            for (let index = 0; index < chunks.length; index += 1) {
                ...
                controller.enqueue(encoder.encode(encodeSseFrame([
                    ["id", chunkId],
                    ["event", "chunk"],
                    [
                        "data",
...
function respondWithTtsBytes(bytes, requestKey, cacheStatus, mode) {
    if (mode === "events") {
...
app.get("/test/billing-gate-check", async (c, next) => {
    if (c.env.ENABLE_TEST_AUTH !== "true") {
        return new Response("Not Found", { status: 404 });
    }
    return next();
}, requireAuth, requireActiveSubscription, (c) => c.json({ ok: true }));
...
app.post("/api/billing/realtime-usage", requireAuth, async (c) => {
    ...
    c.executionCtx.waitUntil(meterFromContext(c.env, c.get("userId"), parsed.usage));
...
app.post("/api/audio/speech", requireAuth, async (c) => {
    try {
        ...
        const validVoice = allowedVoices.includes(voice)
            ? voice
            : "marin";
        ...
        // a hostile float.
        const validSpeed = typeof speed === "number" &&
        ...
        c.executionCtx.waitUntil(meterFromContext(c.env, c.get("userId"), {
            type: "tts",
            model: "gpt-4o-mini-tts",
        ...
        c.executionCtx.waitUntil(c.env.TTS_CACHE.put(key, audioBytes, {
        ...
        if (APICallError.isInstance(error)) {
            ...
            return c.json({ error: "TTS generation failed" }, error.statusCode === 429 ? 429 : 502);
        }
...
app.post("/api/audio/speech/elevenlabs", requireAuth, async (c) => {
    try {
        ...
        const validModel = resolveElevenLabsModelId(typeof model === "string" ? model : undefined);
        const validSpeed = typeof speed === "number" &&
            Number.isFinite(speed) &&
            speed >= 0.25 &&
            speed <= 4.0
            ? speed
            : 1.0;
        const key = await elevenLabsTtsCacheKey(text, validVoice, validModel, validSpeed);
        const cached = await c.env.TTS_CACHE.get(key);
        if (cached !== null) {
            const bytes = await cached.bytes();
            return respondWithTtsBytes(bytes, key, "hit", responseMode);
        ...
        const elevenLabsUrl = new URL(`https://api.elevenlabs.io/v1/text-to-speech/${validVoice}`);
        ...
        if (!speech.ok) {
            ...
            return c.json({ error: "TTS generation failed" }, speech.status === 429 ? 429 : 502);
        }
        c.executionCtx.waitUntil(meterFromContext(c.env, c.get("userId"), {
            type: "tts",
            model: validModel,
...
app.post("/api/realtime/client_secrets", requireAuth, requireActiveSubscription, async (c) => {
    try {
        ...
        const response = await axios.post("https://api.openai.com/v1/realtime/client_secrets", buildRealtimeClientSecretsBody({
            language,
...
app.post("/api/audio/transcribe", requireAuth, async (c) => {
    ...
    let dgResponse;
    try {
        dgResponse = await fetch("https://api.deepgram.com/v1/listen?model=nova-3&smart_format=true&punctuate=true&language=en", {
            method: "POST",
            headers: {
