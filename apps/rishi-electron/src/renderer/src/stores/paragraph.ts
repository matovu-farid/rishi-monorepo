...
export const usePdfParagraphStore = create<PdfParagraphStore>()(
  devtools(
    (set, get) => ({
      ...
      setCurrentParagraphIndex: (index: number) => set({ currentParagraphIndex: index }),
      setCurrentParagraph: (paragraph: ParagraphWithIndex) => set({ currentParagraph: paragraph }),
      ...
      setCurrentViewPages: (pages: number[]) => set({ currentViewPages: pages }),
      setPreviousViewPages: (pages: number[]) => set({ previousViewPages: pages }),
      setNextViewPages: (pages: number[]) => set({ nextViewPages: pages }),
      getCurrentViewParagraphs: () => {
        const { currentViewPages } = get()
        return currentViewPages.map((pageNumber) => get().paragraphs[pageNumber]).flat()
      },
      getNextViewParagraphs: () => {
        const { nextViewPages } = get()
        return nextViewPages.map((pageNumber) => get().paragraphs[pageNumber]).flat()
      },
      getPreviousViewParagraphs: () => {
        const { previousViewPages } = get()
        return previousViewPages.map((pageNumber) => get().paragraphs[pageNumber]).flat()
      }
    }),
    {
