#!/usr/bin/env ruby
# Wire the RishiOnboarding local SwiftPM package into rishi.xcodeproj.
# Idempotent: safe to re-run.
#
# Mirrors apps/apple/scripts/wire-rishi-voice.rb (Phase 10 Plan 10-01).

require 'xcodeproj'

PROJ_PATH = File.expand_path('../rishi/rishi.xcodeproj', __dir__)
PKG_NAME  = 'RishiOnboarding'
...
if ref.nil?
  ...
  puts "[wire-rishi-onboarding] Added local package reference #{PKG_PATH}"
else
  puts "[wire-rishi-onboarding] Local package reference #{PKG_PATH} already present"
end
...
if product_dep.nil?
  ...
  puts "[wire-rishi-onboarding] Added product dependency #{PKG_NAME} to rishi target"
else
  puts "[wire-rishi-onboarding] Product dependency #{PKG_NAME} already on rishi target"
end
...
unless already_linked
  ...
  puts "[wire-rishi-onboarding] Linked #{PKG_NAME} into rishi frameworks build phase"
else
  puts "[wire-rishi-onboarding] #{PKG_NAME} already in rishi frameworks build phase"
end

project.save
puts "[wire-rishi-onboarding] Saved #{PROJ_PATH}"
