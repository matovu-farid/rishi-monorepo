...
desktopRoutes.post("/start", async (c) => {
    ...
    if (!body.success)
        return c.json({ error: "bad_request", issues: body.error.issues }, 400);
...
desktopRoutes.post("/start/complete", async (c) => {
    ...
    const redis = Redis.fromEnv(c.env);
    const stored = (await redis.get(stateKey(body.data.state))) ?? null;
    if (!stored) {
        ...
        const completed = await redis.get(completedKey(body.data.state));
    ...
    await redis.set(resultKey(body.data.state), result, {
    ...
    await redis.set(completedKey(body.data.state), completed, {
...
desktopRoutes.post("/poll", async (c) => {
    ...
    if (!body.success)
        return c.json({ error: "bad_request" }, 400);
    const redis = Redis.fromEnv(c.env);
    const result = await redis.get(resultKey(body.data.state));
    if (!result) {
...
desktopRoutes.post("/cancel", async (c) => {
    ...
    if (!body.success)
        return c.json({ error: "bad_request" }, 400);
    if (body.data.states.length === 0)
        return c.json({ ok: true });
    const redis = Redis.fromEnv(c.env);
    const keys = body.data.states.flatMap((s) => [
        stateKey(s),
        resultKey(s),
        completedKey(s),
    ]);
