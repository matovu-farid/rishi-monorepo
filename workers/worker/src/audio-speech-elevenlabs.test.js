...
vi.mock("./billing/sub-gate", () => ({
    requireActiveSubscription: async (_c, next) => {
        await next();
    },
}));
vi.mock("./middleware", () => ({
...
const ctx = {
    waitUntil: (_p) => undefined,
    passThroughOnException: () => undefined,
};
async function callSpeech(body) {
...
async function computeKey(text, voice, model, speed) {
...
describe("POST /api/audio/speech/elevenlabs", () => {
    it("cache miss calls ElevenLabs, meters, writes to R2, and returns audio", async () => {
        ...
        expect(fetchCalls[0]?.url).toContain("https://api.elevenlabs.io/v1/text-to-speech/JBFqnCBsd6RMkjVDRZzb");
        const parsedBody = JSON.parse(String(fetchCalls[0]?.init?.body ?? "{}"));
        ...
        const meterCall = meterMock.mock.calls[0];
        expect(meterCall[2]).toMatchObject({
            model: "eleven_v3",
        });
        expect(cache.put).toHaveBeenCalledTimes(1);
        const [putKey, putValue] = cache.put.mock.calls[0];
    ...
    it("unsupported legacy preset falls back to the default ElevenLabs voice", async () => {
        ...
        expect(res.status).toBe(200);
        expect(fetchCalls.length).toBe(1);
        expect(fetchCalls[0]?.url).toContain("https://api.elevenlabs.io/v1/text-to-speech/21m00Tcm4TlvDq8ikWAM");
    });
    ...
    it("response_mode=events streams chunk and done SSE frames with stable ids", async () => {
        ...
        const requestId = await computeKey(text, "JBFqnCBsd6RMkjVDRZzb", "eleven_v3", 1.0);
