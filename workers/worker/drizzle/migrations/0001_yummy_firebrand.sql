...
CREATE UNIQUE INDEX `apple_user_idToken_unique` ON `apple_user` (`idToken`);--> statement-breakpoint
DROP TABLE `account`;--> statement-breakpoint
DROP TABLE `siwa_user`;--> statement-breakpoint
...
CREATE TABLE `__new_apple_notifications_logs` (
	...
	FOREIGN KEY (`user_id`) REFERENCES `apple_user`(`user_id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
...
CREATE TABLE `__new_apple_subscriptions` (
	...
	FOREIGN KEY (`user_id`) REFERENCES `apple_user`(`user_id`) ON UPDATE no action ON DELETE cascade
);
--> statement-breakpoint
