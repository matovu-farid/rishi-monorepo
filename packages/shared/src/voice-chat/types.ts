...
/** Book outline. Was `@/lib/api.BookOutline` on electron. */
...
/** Visual content summary. Was `@/lib/visualHeuristic.VisualSummary` on electron. */
...
/** Captured page screenshot. Was `@/modules/pageCapture.CaptureResult` on electron. */
export interface CaptureResult {
  /** A data URL — `data:image/{webp,jpeg,png};base64,…`. */
  dataUrl: string
  width: number
  height: number
  bytes: number
...
/**
 * Structural shape of the RAG service. The electron implementation
 * exposes more (searchText, isIndexed); voice-chat only needs the
 * semantic-search method, so we declare just that.
 */
export interface VoiceChatRagPort {
  searchSemantic(
    query: string,
    bookId: number,
    k: number
...
/**
 * Structural shape of the connectivity service. The electron
 * implementation also exposes `start`/`stop`, but voice-chat never
 * touches them — the parent service owns the lifecycle.
 */
export interface VoiceChatConnectivityPort {
  isOnline(): boolean
...
/** Minimal MediaStream surface used by the activation pipeline. */
...
/**
 * Narrow shape the activation pipeline needs from MediaRecorder.
 * Matches the WHATWG API but lets us mock it in tests AND adapt the
 * mobile-side recorder (which is not the browser's MediaRecorder).
 */
...
/**
 * Backwards-compat alias for the electron `MediaStreamConstraints` shape.
 * Mobile callers can pass `{ audio: true }` or the rich `{ audio: { … } }`
 * form — the shape is intentionally permissive.
 */
export interface VoiceChatMediaConstraints {
  audio:
    | boolean
    | {
...
export interface MediaPort {
  getUserMedia(constraints: VoiceChatMediaConstraints): Promise<MediaStreamLike>
...
export interface VadPort {
   ...
   * `AudioContext` (or the equivalent platform polyfill) is unavailable —
   * the activation pipeline treats `null` as VAD-disabled and proceeds
   * without gating.
   */
...
export interface NetworkPort {
   ...
   * idempotent. MUST NOT make an authenticated or billable request.
   */
...
export interface VoiceChatServiceDeps {
   ...
   * ISO-639-1 code; callers must accept any string and validate downstream.
   */
  getLanguage(): string
  /**
   * Optional Sentry/error-reporting hook. Best-effort — never throws.
   * Default no-op so callers can ignore this port entirely.
   */
  captureError?(err: unknown, context: { operation: string; step: string }): void
}
