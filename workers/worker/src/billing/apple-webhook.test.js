...
function makeDeps(opts = { envelope: null }) {
    ...
    const insertLog = vi.fn(async (row) => {
    ...
    const upsertSub = vi.fn(async (row) => {
        subRows[row.appleTransactionId] = row;
    });
    const updateSubStatus = vi.fn(async (txId, patch) => {
        if (subRows[txId])
            Object.assign(subRows[txId], patch);
    });
    const findUserIdByOriginalTransactionId = vi.fn(async (originalTransactionId) => {
        const match = Object.values(subRows).find((r) => r.appleOriginalTransactionId === originalTransactionId &&
            r.userId &&
            r.userId !== "");
        return match ? match.userId : null;
    });
    const findDevicesByUserId = vi.fn(async (_userId) => {
        return opts.devices ?? [];
    });
    const sendSilentPush = vi.fn(async () => {
        if (opts.apns === "reject")
            throw new Error("apns failed");
    });
    const apns = opts.apns === "set" || opts.apns === "reject"
        ? { sendSilentPush }
        : null;
    const verifyJws = vi.fn(async (jws) => {
...
describe("handleAppleWebhook", () => {
    ...
    it("DID_RENEW: resolves userId from prior verify-receipt row by originalTransactionId", async () => {
        ...
        expect(deps._spy.findUserIdByOriginalTransactionId).toHaveBeenCalledWith("2000000300000001");
        expect(deps._spy.upsertSub).toHaveBeenCalledOnce();
        const row = deps._spy.upsertSub.mock.calls[0][0];
