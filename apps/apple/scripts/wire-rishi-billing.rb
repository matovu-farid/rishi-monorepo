#!/usr/bin/env ruby
# Wire the RishiBilling local SwiftPM package into rishi.xcodeproj.
...
if ref.nil?
  ...
  puts "[wire-rishi-billing] Added local package reference #{PKG_PATH}"
else
  puts "[wire-rishi-billing] Local package reference #{PKG_PATH} already present"
end
...
if product_dep.nil?
  ...
  puts "[wire-rishi-billing] Added product dependency #{PKG_NAME} to rishi target"
else
  puts "[wire-rishi-billing] Product dependency #{PKG_NAME} already on rishi target"
end
...
unless already_linked
  ...
  puts "[wire-rishi-billing] Linked #{PKG_NAME} into rishi frameworks build phase"
else
  puts "[wire-rishi-billing] #{PKG_NAME} already in rishi frameworks build phase"
end

project.save
puts "[wire-rishi-billing] Saved #{PROJ_PATH}"
