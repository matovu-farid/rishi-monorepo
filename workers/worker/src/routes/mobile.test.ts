...
let fakeSession: { user: { id: string }; session: { token: string } } | null = null
const setSession = (s: typeof fakeSession) => {
  fakeSession = s
}
...
async function call(
  routes: typeof mobileRoutes | typeof desktopRoutes,
  path: string,
  init: RequestInit = {}
) {
...
describe("POST /mobile/start/complete", () => {
  it("writes a result record when the web session is valid and state exists", async () => {
    ...
    expect(res.status).toBe(200)
    const stored = redisStore.get(`mobile:result:${state}`) as
      | { session_token: string; user_id: string; code_challenge: string }
      | undefined
...
describe("POST /mobile/start/verify", () => {
  it("returns { session_token } when verifier matches the stored challenge", async () => {
    ...
    expect(res.status).toBe(200)
    const body = (await res.json()) as { session_token: string; user_id: string }
...
describe("Regression: /desktop/start (Electron)", () => {
  ...
  it("still accepts a well-formed oauth-google start and returns state + web_url", async () => {
    ...
    expect(res.status).toBe(200)
    const body = (await res.json()) as { state: string; web_url: string }
