...
import { reportRealtimeUsage, REALTIME_USAGE_PATH, REALTIME_USAGE_CAP, } from "./realtime-usage-client";
...
describe("reportRealtimeUsage — happy path", () => {
    test("POSTs JSON body with all four counters on 2xx", async () => {
        const apiFetch = vi.fn(async () => jsonResponse(200, { ok: true }));
        const result = await reportRealtimeUsage(apiFetch, {
...
describe("reportRealtimeUsage — non-2xx responses", () => {
    test("400 returns ok:false with a reason string, does not throw", async () => {
        const apiFetch = vi.fn(async () => jsonResponse(400, { error: "bad request" }));
        const result = await reportRealtimeUsage(apiFetch, {
            audioInputTokens: 10,
            audioOutputTokens: 0,
            textInputTokens: 0,
    ...
    test("401 returns ok:false and does NOT throw", async () => {
        const apiFetch = vi.fn(async () => jsonResponse(401, {}));
        const result = await reportRealtimeUsage(apiFetch, {
            audioInputTokens: 10,
            audioOutputTokens: 0,
            textInputTokens: 0,
    ...
    test("500 returns ok:false and does NOT throw", async () => {
        const apiFetch = vi.fn(async () => jsonResponse(500, {}));
        const result = await reportRealtimeUsage(apiFetch, {
            audioInputTokens: 10,
            audioOutputTokens: 0,
            textInputTokens: 0,
