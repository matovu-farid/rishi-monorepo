...
vi.mock("framer-motion", () => ({
  motion: {
    ...
    rect: ({
      ...
      rx,
      ry,
    }: React.SVGAttributes<SVGRectElement> & { animate?: unknown }) => (
      <rect
        fill={fill}
        rx={rx}
        ry={ry}
...
describe("SpotlightOverlay", () => {
  ...
  it("renders SVG overlay when target element exists", () => {
    const { getByTestId } = render(
      <SpotlightOverlay targetSelector="test-target" />
    );
  ...
  it("does not render when target element is missing", () => {
    const { queryByTestId } = render(
      <SpotlightOverlay targetSelector="nonexistent-target" />
    );
  ...
  it("applies custom padding", () => {
    const { getByTestId } = render(
      <SpotlightOverlay targetSelector="test-target" padding={16} />
    );
