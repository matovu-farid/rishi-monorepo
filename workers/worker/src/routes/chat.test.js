...
function setSubscription(allowed, status = "active") {
...
vi.mock("@ai-sdk/openai", () => ({
    createOpenAI: (_opts) => ({
        responses: (m) => ({ modelId: m }),
    }),
}));
// ─── Mock billing meter ──────────────────────────────────────────────────────
vi.mock("../billing/meter", () => ({
    meterFromContext: async (..._args) => {
...
vi.mock("../billing/sub-gate", () => ({
    requireActiveSubscription: async (c, next) => {
        if (!subState.allowed) {
            return c.json({
...
describe("POST /api/chat", () => {
    ...
    it("happy path: streams SSE frames matching iOS ChatResponseChunk shape", async () => {
        ...
        const firstObj = JSON.parse(firstDataMatch[1]);
