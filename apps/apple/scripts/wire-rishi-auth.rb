#!/usr/bin/env ruby
# Wire the RishiAuth local SwiftPM package into rishi.xcodeproj.
...
if ref.nil?
  ...
  puts "[wire-rishi-auth] Added local package reference #{PKG_PATH}"
else
  puts "[wire-rishi-auth] Local package reference #{PKG_PATH} already present"
end

# 2) Add product dependency on RishiAuth to the rishi app target (idempotent)
...
if product_dep.nil?
  ...
  puts "[wire-rishi-auth] Added product dependency #{PKG_NAME} to rishi target"
else
  puts "[wire-rishi-auth] Product dependency #{PKG_NAME} already on rishi target"
end
...
unless already_linked
  ...
  puts "[wire-rishi-auth] Linked #{PKG_NAME} into rishi frameworks build phase"
else
  puts "[wire-rishi-auth] #{PKG_NAME} already in rishi frameworks build phase"
end

project.save
puts "[wire-rishi-auth] Saved #{PROJ_PATH}"
