...
function loadEnv(remote: boolean): Env {
  ...
  if (STRIPE_SECRET_KEY.startsWith("sk_live_") && !remote) {
    throw new Error(
      "Refusing to use a live Stripe key without --remote. Pass --remote to run against production.",
    );
  }
  if (
    ...
    throw new Error(
      `STRIPE_SECRET_KEY does not look like a Stripe key: ${STRIPE_SECRET_KEY.slice(0, 8)}…`,
    );
  }
  return { STRIPE_SECRET_KEY };
}

function d1Exec(sqlCmd: string, remote: boolean): string {
  const args = [
    "wrangler",
    "d1",
    "execute",
...
function d1Query<T = Record<string, unknown>>(
  sqlCmd: string,
  remote: boolean,
): T[] {
...
async function realRun(flags: Flags): Promise<void> {
  ...
  if (flags.remote) {
    console.log("\n" + "!".repeat(72));
    console.log(
      `! REMOTE MODE: writing to production D1${isLive ? " + LIVE Stripe" : ""}.`,
    );
  ...
  if (flags.filter) whereParts.push(`(${flags.filter})`);
  const where = whereParts.join(" AND ");
  const query = `SELECT id, email FROM user WHERE ${where} LIMIT ${flags.limit};`;
  console.log(`→ ${query}`);

  const users = d1Query<UserRow>(query, flags.remote);
  ...
  for (const u of users) {
    ...
    try {
      const result: BackfillResult = await backfillOneUser(
        stripe,
        u.id,
        u.email,
        priceId,
        {
          onCustomerEnsured: async (stripeCustomerId) => {
            d1Exec(
              `UPDATE user SET stripe_customer_id = ${sql(stripeCustomerId)} WHERE id = ${sql(u.id)};`,
              flags.remote,
            );
          },
        },
      ...
      console.log(
        `  ✓ ${u.id} → ${result.stripeCustomerId} [${result.steps.join(", ")}]`,
      );
      outcomes.push({
        userId: u.id,
        email: u.email,
  ...
  for (const o of outcomes) {
    console.log(
      `${o.userId.padEnd(28).slice(0, 28)}${o.email.padEnd(32).slice(0, 32)}${o.stripeCustomerId.padEnd(28).slice(0, 28)}${o.steps.padEnd(14).slice(0, 14)}${o.status}`,
    );
