#!/usr/bin/env ruby
# Wire the RishiSync local SwiftPM package into rishi.xcodeproj.
...
if ref.nil?
  ...
  puts "[wire-rishi-sync] Added local package reference #{PKG_PATH}"
else
  puts "[wire-rishi-sync] Local package reference #{PKG_PATH} already present"
end
...
if product_dep.nil?
  ...
  puts "[wire-rishi-sync] Added product dependency #{PKG_NAME} to rishi target"
else
  puts "[wire-rishi-sync] Product dependency #{PKG_NAME} already on rishi target"
end
...
unless already_linked
  ...
  puts "[wire-rishi-sync] Linked #{PKG_NAME} into rishi frameworks build phase"
else
  puts "[wire-rishi-sync] #{PKG_NAME} already in rishi frameworks build phase"
end

project.save
puts "[wire-rishi-sync] Saved #{PROJ_PATH}"
