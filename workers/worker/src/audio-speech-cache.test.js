        ...
        audio = {
            speech: {
                create: vi.fn(async (args) => {
                    ...
                    return {
                        arrayBuffer: async () => speechAudioBytes.buffer.slice(speechAudioBytes.byteOffset, speechAudioBytes.byteOffset + speechAudioBytes.byteLength),
                    };
                }),
            },
        };
 ...
 */
async function computeKey(text, voice, speed) {
...
describe("POST /api/audio/speech — R2 cache gate (Phase 22-01)", () => {
    it("cache miss calls OpenAI, meters, writes to R2, returns X-TTS-Cache: miss", async () => {
        ...
        const [putKey, putValue] = ttsCachePutMock.mock.calls[0];
    ...
    it("pinned hex matches the constant the iOS test asserts", async () => {
        ...
        const fromCanonical = [
            ...new Uint8Array(await crypto.subtle.digest("SHA-256", new TextEncoder().encode(PINNED_CANONICAL))),
        ]
            .map((b) => b.toString(16).padStart(2, "0"))
            .join("");
