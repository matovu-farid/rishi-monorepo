...
export function registerAuthHandlers(): void {
  ...
  ipcMain.handle(
    "auth:saveToken",
    async (_event, token: string, expiresAt: number) => {
      try {
        if (!safeStorage.isEncryptionAvailable()) {
  ...
  ipcMain.handle("auth:checkAuthStatus", async (_event, state: string) => {
    try {
      ...
      const response = await fetch(
        `${WORKER_URL}/api/auth/status/${encodeURIComponent(state)}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  ipcMain.handle("auth:getUser", async (_event, userId: string) => {
    try {
      ...
      // Call worker API
      const response = await fetch(
        `${WORKER_URL}/api/clerk/user/${encodeURIComponent(userId)}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  ipcMain.handle("auth:getDebug", async (_event, state: string) => {
    try {
      const response = await fetch(
        `${WORKER_URL}/api/auth/debug/${encodeURIComponent(state)}`
      );
      if (!response.ok) {
