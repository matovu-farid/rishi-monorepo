...
import { computeOpenAiCostUsd } from "./cost";
...
describe("computeOpenAiCostUsd — tts", () => {
    test("charges per-character at per-1M rate", () => {
        ...
        const cost = computeOpenAiCostUsd({ type: "tts", model: "tts-1", characters: 2000 }, rates);
...
describe("computeOpenAiCostUsd — embedding", () => {
    test("charges per-token at per-1M rate", () => {
        ...
        const cost = computeOpenAiCostUsd({ type: "embedding", model: "text-embedding-3-small", tokens: 500_000 }, rates);
...
describe("computeOpenAiCostUsd — error cases", () => {
    test("throws when model is missing from the rate card", () => {
        ...
        expect(() => computeOpenAiCostUsd({
            type: "chat",
            model: "gpt-unknown",
            inputTokens: 1,
            outputTokens: 1,
        }, rates)).toThrow(/gpt-unknown/);
    });
});
