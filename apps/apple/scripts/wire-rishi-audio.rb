#!/usr/bin/env ruby
# Wire the RishiAudio local SwiftPM package into rishi.xcodeproj.
...
if ref.nil?
  ...
  puts "[wire-rishi-audio] Added local package reference #{PKG_PATH}"
else
  puts "[wire-rishi-audio] Local package reference #{PKG_PATH} already present"
end
...
if product_dep.nil?
  ...
  puts "[wire-rishi-audio] Added product dependency #{PKG_NAME} to rishi target"
else
  puts "[wire-rishi-audio] Product dependency #{PKG_NAME} already on rishi target"
end
...
unless already_linked
  ...
  puts "[wire-rishi-audio] Linked #{PKG_NAME} into rishi frameworks build phase"
else
  puts "[wire-rishi-audio] #{PKG_NAME} already in rishi frameworks build phase"
end

project.save
puts "[wire-rishi-audio] Saved #{PROJ_PATH}"
