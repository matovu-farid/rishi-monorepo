...
export function highlightRange(
  ...
  try {
    ...
    const domRange = rangeCfi.toRange(view.contents.document, rendition.settings.ignoreClass)
    ...
    return Promise.reject(
      new Error(
        'Error highlighting range: ' + (error instanceof Error ? error.message : String(error))
      )
    )
  }
}
...
export function removeHighlight(rendition: Rendition, cfiRange: string) {
  ...
  try {
    ...
    return Promise.reject(
      new Error(
        'Error removing highlight: ' + (error instanceof Error ? error.message : String(error))
      )
    )
  }
}
...
export async function getAllParagraphsForBook(
  ...
  const paragraphs = views
    .map((view) => {
      ...
      const mapping = getMapping(rendition, 0, position.right - position.left, view)
      if (!mapping) return []
  ...
  const chunkData = paragraphs.map((paragraph) => ({
    ...
    id: stringToNumberID(
      bookId + '-' + paragraph.sectionId + '-' + paragraph.startCfi + '-' + paragraph.endCfi
    )
  }))
  return chunkData
}
function reducePraragraphs(
  ...
  return paragraphs.reduce((acc, paragraph) => {
    ...
    const isPreviousParagraphTooShort =
      acc.length > 0 && acc[acc.length - 1].text.length < minLength && direction === 'backward'
...
function _getParagraphsFromRange(
  ...
  try {
    ...
    if (paragraphs.length === 0 && fullText.trim()) {
      try {
        const cfi = new EpubCFI(range, contents.cfiBase, rendition.settings.ignoreClass)
...
function getCurrentLocationPosition(rendition: Rendition) {
  ...
  if (manager.settings.direction === 'rtl') {
    ...
    pageWidth =
      Math.min(globalChapterPosition.right - globalViewPortPosition, manager.layout.width) -
      pixelsUsedByPreviousPages
    localViewPortStartPosition =
      globalViewPortPosition - globalChapterPosition.left + pixelsUsedByPreviousPages
...
function clampedToChapterLocalDimentions(localPosition: number, chapter: View) {
  const localCurrentChapterEnd = chapter.position().right - chapter.position().left
...
async function _getAdjacentViewParagraphs(
  ...
  if (secondChapter) {
    if (direction === 'next') {
      ...
      const secondChapterLocalEnd = secondChapter.position().right - secondChapter.position().left
  ...
  const paragraphsFromPositions = createParagraphsFromPostions(positions, rendition)
...
export async function getNextViewParagraphs(rendition: Rendition): Promise<ParagraphWithCFI[]> {
...
function createParagraphsFromPostions(
  ...
  for (const position of positions) {
    const mapping = getMapping(rendition, position.start, position.end, position.view)
