#!/usr/bin/env ruby
# Wire the RishiLibrary local SwiftPM package into rishi.xcodeproj.
# Idempotent: safe to re-run.
#
# Mirrors apps/apple/scripts/wire-rishi-db.rb (Phase 2 Plan 02-01).

require 'xcodeproj'

PROJ_PATH = File.expand_path('../rishi/rishi.xcodeproj', __dir__)
PKG_NAME  = 'RishiLibrary'
...
if ref.nil?
  ...
  puts "[wire-rishi-library] Added local package reference #{PKG_PATH}"
else
  puts "[wire-rishi-library] Local package reference #{PKG_PATH} already present"
end

# 2) Add product dependency on RishiLibrary to the rishi app target (idempotent)
...
if product_dep.nil?
  ...
  puts "[wire-rishi-library] Added product dependency #{PKG_NAME} to rishi target"
else
  puts "[wire-rishi-library] Product dependency #{PKG_NAME} already on rishi target"
end
...
unless already_linked
  ...
  puts "[wire-rishi-library] Linked #{PKG_NAME} into rishi frameworks build phase"
else
  puts "[wire-rishi-library] #{PKG_NAME} already in rishi frameworks build phase"
end

project.save
puts "[wire-rishi-library] Saved #{PROJ_PATH}"
