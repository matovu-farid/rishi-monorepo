#!/usr/bin/env ruby
# Wire the RishiReader local SwiftPM package into rishi.xcodeproj.
# Idempotent: safe to re-run.
#
# Mirrors apps/apple/scripts/wire-rishi-library.rb (Phase 4 Plan 04-01).

require 'xcodeproj'

PROJ_PATH = File.expand_path('../rishi/rishi.xcodeproj', __dir__)
PKG_NAME  = 'RishiReader'
...
if ref.nil?
  ...
  puts "[wire-rishi-reader] Added local package reference #{PKG_PATH}"
else
  puts "[wire-rishi-reader] Local package reference #{PKG_PATH} already present"
end
...
if product_dep.nil?
  ...
  puts "[wire-rishi-reader] Added product dependency #{PKG_NAME} to rishi target"
else
  puts "[wire-rishi-reader] Product dependency #{PKG_NAME} already on rishi target"
end
...
unless already_linked
  ...
  puts "[wire-rishi-reader] Linked #{PKG_NAME} into rishi frameworks build phase"
else
  puts "[wire-rishi-reader] #{PKG_NAME} already in rishi frameworks build phase"
end

project.save
puts "[wire-rishi-reader] Saved #{PROJ_PATH}"
