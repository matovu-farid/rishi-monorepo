...
describe("checkBillingGate — throws on BILLING_INACTIVE", () => {
    it("throws BillingInactiveError on 402 + { code: BILLING_INACTIVE }", async () => {
        const res = jsonResponse({ code: "BILLING_INACTIVE", subscriptionStatus: "canceled" }, 402);
        await expect(checkBillingGate(res)).rejects.toBeInstanceOf(BillingInactiveError);
    });
    it("propagates subscriptionStatus from the response body", async () => {
        ...
        try {
            ...
            expect(err.subscriptionStatus).toBe("canceled");
            expect(err.code).toBe("BILLING_INACTIVE");
        }
    });
    it("handles null subscriptionStatus", async () => {
        const res = jsonResponse({ code: "BILLING_INACTIVE", subscriptionStatus: null }, 402);
        try {
            await checkBillingGate(res);
            throw new Error("checkBillingGate should have thrown");
        }
        catch (err) {
            expect(err.subscriptionStatus).toBeNull();
        }
    });
});
