    ...
    for (const change of body.changes) {
      ...
      if (change.kind === "chapter_index") {
        ...
        const parentUpsert = tx.insert(chapterIndexes).select(tx.select({
          ...
          contentVersion: sql<string>`${parent.contentVersion}`.as("content_version"),
          status: sql<string>`${parent.status}`.as("status"),
          modelIdentifier: sql<string>`${parent.modelIdentifier}`.as("model_identifier"),
          modelVersion: sql<string>`${parent.modelVersion}`.as("model_version"),
          completedCount: sql<number>`${parent.completedCount}`.as("completed_count"),
          totalCount: sql<number>`${parent.totalCount}`.as("total_count"),
          errorMessage: sql<string | null>`${parent.errorMessage}`.as("error_message"),
          createdAt: sql<Date>`${parent.createdAt}`.as("created_at"),
          updatedAt: sql<Date>`${parent.updatedAt}`.as("updated_at"),
        ...
        const childStatements = p.chapters.map((chapter) => tx.insert(chapterIndexChapters).select(tx.select({
          id: sql<string>`${`${userId}/${bookId}/${p.content_version}/${chapter.id}`}`.as("id"),
          userId: sql<string>`${userId}`.as("user_id"),
          bookId: sql<string>`${bookId}`.as("book_id"),
          contentVersion: sql<string>`${p.content_version}`.as("content_version"),
          chapterId: sql<string>`${chapter.id}`.as("chapter_id"),
          sourcePosition: sql<number>`${chapter.source_position}`.as("source_position"),
          name: sql<string>`${chapter.name}`.as("name"),
          summary: sql<string>`${chapter.summary}`.as("summary"),
          createdAt: sql<Date>`${pushedUpdatedAtMs}`.as("created_at"),
          updatedAt: sql<Date>`${pushedUpdatedAtMs}`.as("updated_at"),
