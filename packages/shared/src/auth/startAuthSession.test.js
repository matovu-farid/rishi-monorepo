...
describe('startAuthSession', () => {
    it('POSTs { code_challenge, provider } to /mobile/start and returns { authUrl, state, verifier }', async () => {
        const mock = makeFetchMock(async () => new Response(JSON.stringify({
            state: 'srv-state-123',
    ...
    it('defaults provider to google when not specified', async () => {
        const mock = makeFetchMock(async () => new Response(JSON.stringify({ state: 's', authUrl: 'https://x/' }), { status: 200 }));
    ...
    it('throws when /mobile/start returns non-2xx', async () => {
        const mock = makeFetchMock(async () => new Response(JSON.stringify({ error: 'bad_request' }), { status: 400 }));
        await expect(startAuthSession({
            apiUrl: 'https://api.example.com',
            fetch: mock.fn,
...
describe('completeAuthSession', () => {
    it('POSTs { state, code_verifier } to /mobile/start/verify and returns the session token', async () => {
        const mock = makeFetchMock(async () => new Response(JSON.stringify({ session_token: 'sess-abc', user_id: 'user-1' }), { status: 200, headers: { 'Content-Type': 'application/json' } }));
    ...
    it('throws on 403 pkce_mismatch', async () => {
        const mock = makeFetchMock(async () => new Response(JSON.stringify({ error: 'pkce_mismatch' }), { status: 403 }));
        await expect(completeAuthSession({
            apiUrl: 'https://api.example.com',
            state: 's',
            verifier: 'v',
            fetch: mock.fn,
    ...
    it('throws on 410 expired', async () => {
        const mock = makeFetchMock(async () => new Response(JSON.stringify({ error: 'expired' }), { status: 410 }));
        await expect(completeAuthSession({
            apiUrl: 'https://api.example.com',
            state: 's',
            verifier: 'v',
