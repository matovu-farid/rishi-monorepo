...
import { user as userTable, appleSubscriptions, subscription, } from "@rishi/shared/schema";
...
export async function createAuth(env) {
    ...
    const appleConfigured = Boolean(env.APPLE_SIWA_CLIENT_ID &&
                    ...
                    return [
                        stripe({
                            ...
                            onCustomerCreate: async ({ stripeCustomer }, request) => {
                                ...
                                await ensureCreditAndSubscription(stripeClient, stripeCustomer.id, ids.priceId, ip);
                            },
                            onEvent: async (event) => {
                                ...
                                if (!row?.email)
                                    return;
                                await sendPaymentFailedEmail(event, { email: row.email, name: row.name ?? null }, {
