...
import { buildTestKit } from "./jws-fixtures";
let kit;
beforeAll(async () => {
...
describe("verifyAppleJWS — happy path", () => {
    it("verifies a valid JWS signed by the test root chain", async () => {
        ...
        const payload = await verifyAppleJWS(jws, { rootCa: kit.rootDer });
    ...
    it("decodes x5c entries as base64 STANDARD, not base64url (Pitfall 1)", async () => {
        ...
        const header = JSON.parse(Buffer.from(jws.split(".")[0], "base64url").toString("utf-8"));
        const x5c0 = header.x5c[0];
...
describe("verifyAppleJWS — header rejects", () => {
    it("throws JWSInvalid('payload') when payload is tampered", async () => {
        ...
        await expect(verifyAppleJWS(tampered, { rootCa: kit.rootDer })).rejects.toMatchObject({ name: "JWSInvalid", reason: "payload" });
    });
    it("throws JWSInvalid('root_mismatch') when root pin does not match x5c[2]", async () => {
        const jws = await kit.signFixture({ sub: "tx-123" });
        const wrongRoot = new Uint8Array(64);
        await expect(verifyAppleJWS(jws, { rootCa: wrongRoot })).rejects.toMatchObject({ name: "JWSInvalid", reason: "root_mismatch" });
    });
    it("throws JWSInvalid('alg') when header alg is not ES256", async () => {
        const jws = await kit.signFixture({ sub: "tx-123" }, { alg: "HS256" });
        await expect(verifyAppleJWS(jws, { rootCa: kit.rootDer })).rejects.toMatchObject({ name: "JWSInvalid", reason: "alg" });
    });
    it("throws JWSInvalid('x5c') when header x5c is missing", async () => {
        const jws = await kit.signFixture({ sub: "tx-123" }, { omitX5c: true });
        await expect(verifyAppleJWS(jws, { rootCa: kit.rootDer })).rejects.toMatchObject({ name: "JWSInvalid", reason: "x5c" });
    });
});
/**
...
describe("verifyAppleJWS — X.509 chain validation (security review)", () => {
    it("rejects forged-leaf attack (attacker leaf at x5c[0], real Apple root at x5c[2])", async () => {
        ...
        // alone does not.
        await expect(verifyAppleJWS(jws, { rootCa: kit.rootDer })).rejects.toMatchObject({ name: "JWSInvalid", reason: "chain" });
    });
    ...
    it("rejects expired leaf certificate", async () => {
        ...
        await expect(verifyAppleJWS(jws, { rootCa: kit.rootDer })).rejects.toMatchObject({ name: "JWSInvalid", reason: "expired" });
    });
