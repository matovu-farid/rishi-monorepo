...
describe("embedWithFallback", () => {
  ...
  it("should fall back to server when on-device fails", async () => {
    vi.mocked(window.electron.embed).mockRejectedValue(
  ...
  it("should use dev bypass when no auth token available", async () => {
    ...
    vi.mocked(window.electron.getDevBypassSecret).mockResolvedValue(
      "dev-secret",
    );
  ...
  it("should throw when both on-device and server fail", async () => {
    ...
    await expect(embedWithFallback(sampleParams)).rejects.toThrow(
      "Server embed failed: 500",
    );
  });
