...
vi.mock("./billing/sub-gate", () => ({
    requireActiveSubscription: async (_c, next) => next(),
}));
...
async function callClientSecretsPOST(body) {
...
describe("buildRealtimeClientSecretsBody", () => {
    ...
    it("propagates the language hint into transcription.language", () => {
        expect(buildRealtimeClientSecretsBody({ language: "fr" }).session.audio.input.transcription.language).toBe("fr");
        expect(buildRealtimeClientSecretsBody({ language: "ja" }).session.audio.input.transcription.language).toBe("ja");
    });
...
describe("POST /api/realtime/client_secrets handler", () => {
    ...
    it("falls back to a local_<uuid> session_id when OpenAI omits id", async () => {
        ...
        expect(body.session_id).toMatch(/^local_/);
    });
    ...
    it("forwards body language into the OpenAI request transcription block", async () => {
        ...
        expect(res.status).toBe(200);
        const capturedBody = openaiCaptured.body;
