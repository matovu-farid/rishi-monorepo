/**
 * State-graph coverage tests for `voiceChatMachine` (shared package copy).
 *
 * Mirrors the structure of `playerMachine.coverage.test.ts` in rishi-electron.
 * The hand-rolled `actor.send(...)` tests in `machine.test.ts` already cover
 * the well-known transitions; this file complements them by:
 *
