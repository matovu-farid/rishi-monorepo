...
import type { FormatsPort, BookDataParsed } from "./types";
...
export function makeFormats(opts?: {
  ...
  formats: FormatsPort;
  calls: Array<{ method: keyof FormatsPort; path: string }>;
} {
  const calls: Array<{ method: keyof FormatsPort; path: string }> = [];
  const formats: FormatsPort = {
...
describe("dispatchFormatExtraction", () => {
  it("routes .epub to formats.getBookData and returns the parsed data + format", async () => {
    ...
    expect(calls).toEqual([
      { method: "getBookData", path: "/userData/book.epub" },
    ]);
  });
});
...
describe("dispatchFormatExtraction — extension routing", () => {
  ...
  it("throws UnsupportedFormatError for unknown extensions", async () => {
    const { formats, calls } = makeFormats();
    await expect(
      dispatchFormatExtraction(formats, "/tmp/notes.txt"),
  ...
  it("is case-insensitive on the extension", async () => {
    ...
    expect(calls).toEqual([
      { method: "getBookData", path: "/tmp/BOOK.EPUB" },
    ]);
  });
});
