...
/**
 * RMS-based local VAD. Verbatim port from
 * `apps/rishi-electron/src/renderer/src/services/voice-chat/local-vad.ts`.
 *
 * The implementation reads a global `AudioContext` constructor and degrades
 * to `null` when absent — that's the platform escape hatch. Electron uses
 * the browser's Web Audio API; React Native uses `react-native-audio-api`
 * (when installed) which polyfills the same global. If neither is present,
