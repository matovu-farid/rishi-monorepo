...
export interface StartAuthSessionArgs {
  apiUrl: string
  /** Defaults to `'google'`. */
  provider?: AuthProvider
  /** Optional client-supplied state. Worker mints one if omitted. */
  state?: string
...
export interface StartAuthSessionResult {
  /** URL the caller opens in an in-app browser to begin OAuth. */
  authUrl: string
  /** Opaque correlation id, also embedded in `authUrl`. Round-trips on the deep link. */
  state: string
  /** PKCE verifier — keep in memory until the deep link comes back, then pass to `completeAuthSession`. */
  verifier: string
}

export interface CompleteAuthSessionArgs {
  apiUrl: string
  state: string
  verifier: string
  /** Override for tests. */
...
export async function startAuthSession(
  args: StartAuthSessionArgs,
): Promise<StartAuthSessionResult> {
  const fetchImpl = args.fetch ?? fetch
  const provider: AuthProvider = args.provider ?? 'google'
  const { code_verifier, code_challenge } = await generatePkcePair()

  const body: Record<string, unknown> = { code_challenge, provider }
...
export async function completeAuthSession(
  ...
  return (await res.json()) as CompleteAuthSessionResult
}
