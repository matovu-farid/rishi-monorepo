...
const { booksStore, highlightsStore, conversationsStore, messagesStore, bookmarksStore, BOOK_COLS, HIGHLIGHT_COLS, CONV_COLS, MSG_COLS, BOOKMARK_COLS, } = vi.hoisted(() => {
    function mkCols(table, names) {
        const out = {};
        for (const n of names)
            out[n] = { __table: table, __col: n };
        return out;
    }
    return {
        booksStore: [],
...
vi.mock("drizzle-orm", () => ({
    ...
    asc: (col) => ({ table: col.__table, col: col.__col, dir: "asc" }),
    desc: (col) => ({ table: col.__table, col: col.__col, dir: "desc" }),
    max: (col) => ({ __agg: "max", col }),
    sql: (..._args) => ({ __sql: true }),
    getTableColumns: (_t) => ({}),
}));
...
describe("POST /api/sync/push — bookmark kind", () => {
    ...
    it("cross-user isolation: a bookmark for user A is not written/visible under user B", async () => {
        ...
        const aliceRow = bookmarksStore.find((r) => r.id === UUID_BM && r.userId === "user_alice");
        const bobRow = bookmarksStore.find((r) => r.id === UUID_BM && r.userId === "user_bob");
