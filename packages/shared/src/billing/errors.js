...
export class BillingInactiveError extends Error {
    ...
    subscriptionStatus;
    constructor(subscriptionStatus, message) {
        super(message ??
    ...
    static isInstance(err) {
        if (err instanceof BillingInactiveError)
            return true;
        return (typeof err === "object" &&
            err !== null &&
            err.code === "BILLING_INACTIVE");
    }
}
/**
 ...
 */
export function isBillingInactiveResponse(status, body) {
    if (status !== 402)
        return false;
    if (typeof body !== "object" || body === null)
        return false;
    return body.code === "BILLING_INACTIVE";
}
