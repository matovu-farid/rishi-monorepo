...
// (background-page.tsx). chunk_data uses INSERT OR IGNORE keyed on this id, and
// HNSW vector labels must match, so the formula MUST stay stable.
const chunkId = (pageNumber: number, bookId: number, index: number): number =>
...
export const indexBookProgram = (deps: IndexBookDeps): Effect.Effect<void, Error> => {
  ...
  const ordered = pagesAroundCenter(deps.startPage ?? 1, deps.numPages).filter(
    (p) => !skip.has(p)
  )
