...
export function registerVectorHandlers(): void {
  ipcMain.handle("vectors:embed", async (_event, params: unknown[]) => {
    try {
      return await generateEmbeddings(params as Array<{ text: string; metadata: { id: number; pageNumber: number; bookId: number } }>);
    } catch (error) {
      throw new Error(
        `Failed to embed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  });
