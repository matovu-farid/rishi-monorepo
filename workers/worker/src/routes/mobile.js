...
const ALLOWED_PROVIDERS = ["google", "apple", "password"];
...
mobileRoutes.post("/start/complete", async (c) => {
    ...
    const redis = Redis.fromEnv(c.env);
    const stored = (await redis.get(stateKey(parsed.data.state))) ?? null;
    if (!stored)
    ...
    await redis.set(resultKey(parsed.data.state), result, {
...
mobileRoutes.get("/start/complete", async (c) => {
    ...
    if (!parsed.success)
        return c.json({ error: "bad_request" }, 400);
    const redis = Redis.fromEnv(c.env);
    const result = await redis.get(resultKey(parsed.data.state));
    if (!result)
        return c.json({ error: "state_expired" }, 400);
