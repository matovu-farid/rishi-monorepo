...
- **Subagents must NOT run `xcodebuild rishi`** (600s watchdog kills agents).
  ...
  - **`RishiReaderTests` is NOT runnable headless** — neither the `RishiReader` scheme (no test action) nor the `rishi` scheme (not a member of its test plan) runs it, and `swift test` is blocked by the Readium macOS-platform conflict. To get a deterministic RishiReader-logic test you CAN run, put it in the **app** target `rishiTests` (runs via `-scheme rishi`, plain `import RishiReader` = public API only; make the seam `public`). GOTCHA: a NEW file must go at the **root** of `rishiTests/` — a brand-new SUBfolder (e.g. `rishiTests/Reader/`) is not rescanned by incremental `xcodebuild` even though the group is filesystem-synchronized, so it silently compiles to 0 tests. After adding, the FIRST run compiles the bundle; RE-RUN once for `-only-testing` to discover the suite.
...
- **Bug 4 (page-boundary halt) — FIXED (`9b651ecf5`, test `b0228cc36`).** Symptom: crossing a Readium page boundary (auto-advance OR Next button) highlighted+partially played the new page's first paragraph, then halted. Root cause (confirmed via live os_log): an animated `go(to:)` from `navigateToReadAloudParagraph` delivers its `locationDidChange` ~300ms AFTER it returns, and the read-aloud target locator is text-anchored with NO progression, so within a resource a programmatic page turn and a user swipe are indistinguishable from a single callback. The old per-call `isProgrammaticNavigation` flag reset synchronously after `go(to:)`, so the delayed callback was tagged USER and fired `onUserNavigation -> stopReadAloud`. Fix: session-scoped suppression — `EPUBNavigatorCoordinator.isFollowingReadAloud` (set by `EPUBReaderScreen` from the active-passage binding); while following, every location change is auto-follow and does not stop playback. Tradeoff: a manual page-turn during read-aloud no longer stops it (use Stop). Guards: deterministic `rishiTests/EPUBNavigatorCoordinatorFollowTests` (3 tests, green) + integration `rishiUITests/ReadAloudNextParagraphUITests.testCrossingPageBoundaryKeepsPlaying` (12-press Next sweep, asserts `tts-stop` persists; green ~68s). Device re-confirm still welcome but the simulator XCUITest now reproduces+guards it.
- **Feature 5 device-verify**: prev/repeat/next at page boundaries. User confirmed next/prev audio works; the Bug 4 fix also covers the page-crossing case for these (same auto-follow path).
