...
**Status:** fixed
**Commit:** ea768346
**Notes:** Captured `window.electron` at module load into `originalElectron` and added a symmetric `afterEach` that restores it, closing the leak window. Added two tests: (1) "keeps the listener installed when a handler throws synchronously" — asserts `listener!()` throws but a subsequent `listener!()` for a different command still dispatches; (2) "routes commands to distinct handlers in dispatch order" — 4 calls across 3 handlers, asserts call counts and an explicit order array. Did NOT add the `windowIdentity.kind` filter test per Reviewer-1's rejection of that angle (filtering lives in main, not in this hook). Production untouched; suite goes from 3 to 5 tests.
