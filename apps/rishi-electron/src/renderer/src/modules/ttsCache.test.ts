...
describe('TTSCache', () => {
  beforeEach(() => {
    ...
    vi.mocked(window.electron.getAppDataPath).mockResolvedValue('/tmp/test-app-data')
  ...
  it('should initialize cache directory on construction', async () => {
    ...
    expect(window.electron.mkdir).toHaveBeenCalledWith('/tmp/test-app-data/tts-cache')
  })
  ...
  it('should save audio blob to disk', async () => {
    vi.mocked(window.electron.exists).mockResolvedValueOnce(true) // book dir exists
    vi.mocked(window.electron.getDirSize).mockResolvedValue(0)
  ...
  it('should trigger cleanup when cache exceeds 80% threshold', async () => {
    ...
    vi.mocked(window.electron.getDirSize).mockResolvedValue(450 * 1024 * 1024)
    vi.mocked(window.electron.getCacheFileStats).mockResolvedValue([
    ...
    vi.mocked(window.electron.exists).mockResolvedValueOnce(true) // book dir check
  ...
  it('should also save under text-hash key when text is provided', async () => {
    vi.mocked(window.electron.exists)
      .mockResolvedValueOnce(true) // book dir for CFI key (memoized after this)
      .mockResolvedValueOnce(false) // text-hash file does not exist yet → triggers copyFile
    vi.mocked(window.electron.getDirSize).mockResolvedValue(0)
  ...
  it('should fall back to text-hash lookup when CFI lookup misses', async () => {
    vi.mocked(window.electron.exists)
      .mockResolvedValueOnce(true) // book dir for CFI (memoized after this)
      .mockResolvedValueOnce(false) // CFI file does not exist
      .mockResolvedValueOnce(true) // text-hash file exists (book dir skipped — memoized)

    const { ttsCache } = await import('./ttsCache')

    const result = await ttsCache.getCachedAudio('book1', 'cfi-unknown', 'Hello world')
    expect(result.exists).toBe(true)
  })
})
