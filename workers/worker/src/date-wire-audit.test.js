...
const NON_DATE_NUMERIC_ROUTES = [
    {
        path: "POST + GET /api/sync/conversations",
        source: "routes/conversations.ts",
        reason: "iOS ConversationsSyncAPI decodes updated_at/created_at as Int64 ms-epoch (Phase 16-02). Not a Swift Date field.",
    },
    {
        path: "POST + GET /api/sync/messages",
        source: "routes/messages.ts",
        reason: "iOS MessagesSyncAPI decodes updated_at/created_at as Int64 ms-epoch (Phase 16-02). Not a Swift Date field.",
    },
];
...
const ISO_STRING_ALLOWED_SOURCES = [
    ...
    describe("static: canonical conversion constant present per route", () => {
        for (const route of IOS_DECODED_DATE_ROUTES) {
            it(`${route.source} contains the canonical constant 978_307_200_000`, () => {
                ...
                const hasConstant = src.includes("978_307_200_000") || src.includes("978307200000");
                expect(hasConstant, `Date-emitting route ${route.path} (${route.source}) emits ${route.dateFieldsEmitted.join(", ")} ` +
                    ...
                    "iOS WorkerClient.swift:96 .deferredToDate.").toBe(true);
            });
        }
    });
    describe("static: no raw toISOString() Date in iOS-decoded JSON bodies", () => {
        for (const route of IOS_DECODED_DATE_ROUTES) {
            it(`${route.source} contains no unguarded .toISOString() Date wire emission`, () => {
                ...
                if (ISO_STRING_ALLOWED_SOURCES.includes(route.source))
                    return;
                const src = readSource(route.source);
                ...
                for (const [lineIndex, line] of src.split("\n").entries()) {
                    ...
                    if (trimmed.startsWith("//") || trimmed.startsWith("*"))
                        continue;
                    if (trimmed.includes("WIRE-ISO8601-PAYLOAD"))
                        continue;
                    if (trimmed.includes(".toISOString()")) {
                        offendingLines.push(`${lineIndex + 1}: ${trimmed}`);
                    }
                }
                expect(offendingLines, `${route.source} emits .toISOString() — would break iOS .deferredToDate decode. ` +
                    `Offending lines:\n${offendingLines.join("\n")}`).toEqual([]);
            });
        }
    });
    ...
    describe("static: ISO-string allow-list is honest", () => {
        it("every allow-listed source actually contains toISOString (no dead entries)", () => {
            for (const allowed of ISO_STRING_ALLOWED_SOURCES) {
                ...
                expect(src.includes(".toISOString()"), `Allow-list entry ${allowed} has no .toISOString() — entry is stale, remove it.`).toBe(true);
            }
        });
    });
