...
- Keep one versioned public API Worker across production and development. Do not create a separate dev API Worker or let API contracts diverge; Wrangler local/`--remote` is the development execution path. The sharing Worker is only the internal WebSocket service required by shared reading.
