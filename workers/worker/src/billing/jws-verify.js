...
export async function verifyAppleJWS(jws, opts = {}) {
    ...
    // attack surface.
    const x5cRaw = header.x5c;
    if (!Array.isArray(x5cRaw) || x5cRaw.length < 3) {
        throw new JWSInvalid("x5c", `x5c header missing or has fewer than 3 entries (got ${Array.isArray(x5cRaw) ? x5cRaw.length : "none"})`);
    }
    const x5c = x5cRaw;
    ...
    try {
        ...
        if (!intSigOk) {
            throw new JWSInvalid("chain", "intermediate cert not signed by pinned root");
        }
    }
    catch (e) {
    ...
    if (!isWithinValidity(leafCert, now)) {
        throw new JWSInvalid("expired", `leaf cert outside validity window (${leafCert.notBefore.toISOString()} .. ${leafCert.notAfter.toISOString()})`);
    }
