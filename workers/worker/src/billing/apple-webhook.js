...
import { appleSubscriptions, appleNotificationsLog, devices as devicesTable, } from "@rishi/shared/schema";
 ...
 */
export async function handleAppleWebhook(input) {
    ...
    if (tx) {
        try {
            ...
            await input.deps.db.markLogProcessed(envelope.notificationUUID, String(e));
 ...
 */
async function dispatch(envelope, tx, deps) {
 ...
 */
async function emitEntitlementChangedPush(originalTransactionId, deps) {
    const userId = (await deps.db.findUserIdByOriginalTransactionId(originalTransactionId)) ??
        "";
    if (!userId) {
        console.log("[apple-webhook] REFUND/REVOKE: no resolvable userId; skipping push");
        return;
    }
    if (!deps.apns) {
        console.warn("[apple-webhook] REFUND/REVOKE: APNs not configured; skipping silent push");
        return;
    }
    const apns = deps.apns;
    const userDevices = await deps.db.findDevicesByUserId(userId);
    if (userDevices.length === 0) {
        console.log(`[apple-webhook] REFUND/REVOKE: user ${userId} has no devices; skipping push`);
        return;
    }
    ...
    for (const d of userDevices) {
        try {
            ...
            console.error(`[apple-webhook] silent push failed for device ${d.deviceToken}: ${String(e)}`);
        }
    }
}
...
export function registerAppleWebhookRoute(app) {
    app.post("/api/billing/apple-webhook", async (c) => {
        ...
        const apns = apnsKeyP8 && apnsKeyId && apnsTeamId
            ? createApnsSender({
                keyP8: apnsKeyP8,
                keyId: apnsKeyId,
                teamId: apnsTeamId,
            ...
            : null;
        const deps = {
            verifyJws: (jws) => verifyAppleJWS(jws),
            apns,
            db: {
                insertLog: async (row) => {
                ...
                findUserIdByOriginalTransactionId: async (originalTransactionId) => {
                    ...
                    const row = await db
                        ...
                        .where(and(eq(appleSubscriptions.appleOriginalTransactionId, originalTransactionId), ne(appleSubscriptions.userId, "")))
