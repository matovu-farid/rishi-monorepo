...
function getLimits(env) {
    return {
        ...
        perUserBytes: Number(env.BOOK_MAX_USER_BYTES) || DEFAULT_BOOK_MAX_USER_BYTES,
    };
}
 ...
 */
function isR2KeySafe(r2Key, expectedPrefix) {
...
uploadRoutes.post("/upload-url", requireAuth, async (c) => {
    ...
    const expiresAtSeconds = (Date.now() + UPLOAD_URL_EXPIRES_SEC * 1000 - REFERENCE_DATE_OFFSET_MS) /
        1000;
    return c.json({
        url: signedUrl,
...
uploadRoutes.post("/download-url", requireAuth, async (c) => {
    ...
    // Date wire format = seconds since 2001-01-01 reference date — see
    // routes/changes.ts and routes/devices.test.ts.
    const expiresAtSeconds = (Date.now() + DOWNLOAD_URL_EXPIRES_SEC * 1000 - REFERENCE_DATE_OFFSET_MS) /
        1000;
    return c.json({
        url: signedUrl,
        expires_at: expiresAtSeconds,
