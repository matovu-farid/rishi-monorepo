...
describe("pdfStore", () => {
  ...
  it("should navigate to next page", () => {
    usePdfStore.setState({ pageNumber: 3, pageCount: 10, bookNavigationState: BookNavigationState.Idle });
  ...
  it("should not go below page 1", () => {
    usePdfStore.setState({ pageNumber: 1, pageCount: 10, bookNavigationState: BookNavigationState.Idle });
    usePdfStore.getState().previousPage();
    expect(usePdfStore.getState().pageNumber).toBe(1);
  });

  it("should not go above page count", () => {
    usePdfStore.setState({ pageNumber: 10, pageCount: 10, bookNavigationState: BookNavigationState.Idle });
    usePdfStore.getState().nextPage();
    expect(usePdfStore.getState().pageNumber).toBe(10);
  });

  it("should handle dual page navigation", () => {
    usePdfStore.setState({ pageNumber: 3, pageCount: 10, isDualPage: true, bookNavigationState: BookNavigationState.Idle });
    usePdfStore.getState().nextPage();
    expect(usePdfStore.getState().pageNumber).toBe(5);
  });
