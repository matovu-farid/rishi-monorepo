// packages/shared/src/machines/navigationHistory/navigationHistoryMachine.ts
//
// Shared port of
// `apps/rishi-electron/src/renderer/src/machines/navigationHistory/navigationHistoryMachine.ts`.
// Only behavioural difference: `resumeMap` is a plain
// `Record<string, AnchorPoint>` instead of a `Map` so XState snapshots remain
// JSON-serializable (REVIEW-01 MINOR-05).
...
export const navigationHistoryMachine = setup({
  ...
  actions: {
    ...
    captureResumeAnchor: assign(({ context }) => {
      ...
      return {
        resumeMap: {
          ...context.resumeMap,
          [pageKey(context.currentPage)]: anchor
        }
      }
    }),
    ...
    emitResume: emit(({ context, event }): NavigationHistoryEmitted => {
      ...
      const anchor = context.resumeMap[pageKey(event.position)]
      if (!anchor) {
  ...
  guards: {
    ...
    hasResumeAnchor: ({ context, event }) => {
      ...
      return pageKey(event.position) in context.resumeMap
    }
  },
  delays: {
