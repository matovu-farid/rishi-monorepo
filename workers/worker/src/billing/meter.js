...
import { METER_EVENT_NAME, usdToMicroDollars, } from "@rishi/shared/billing/stripe-config";
import { user } from "@rishi/shared/schema";
import { computeOpenAiCostUsd, } from "@rishi/shared/billing/cost";
 ...
 */
export async function reportMeterEvent(stripe, stripeCustomerId, microDollarsCost) {
    if (!stripe)
        return;
    if (!stripeCustomerId)
        return;
    if (microDollarsCost <= 0)
        return;
 ...
 */
export async function meterFromContext(env, userId, usage) {
