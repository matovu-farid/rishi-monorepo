...
import { AppStoreServerAPIClient, Environment, SignedDataVerifier, } from "../app-store-server-library-node";
async function parseR2TextFiles(fileName, schema, bucket) {
...
async function parseR2JsonFiles(fileName, schema, bucket) {
    const file = await bucket.get(fileName);
    if (!file) {
        throw new Error(`File not found in R2: ${fileName}`);
    }
...
const getClientEffect = Effect.gen(function* () {
    ...
    const appKey = yield* Effect.tryPromise(() => parseR2JsonFiles(Data.appKey.fileName, Data.appKey.schema, bucket));
    ...
    const encodedKey = yield* Effect.tryPromise(() => parseR2TextFiles(Data.subscriptionKey.fileName, Data.subscriptionKey.schema, bucket));
...
export function verifySignedTransaction(transactionId) {
    return Effect.gen(function* () {
