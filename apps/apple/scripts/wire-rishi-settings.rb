#!/usr/bin/env ruby
# Wire the RishiSettings local SwiftPM package into rishi.xcodeproj.
...
if ref.nil?
  ...
  puts "[wire-rishi-settings] Added local package reference #{PKG_PATH}"
else
  puts "[wire-rishi-settings] Local package reference #{PKG_PATH} already present"
end
...
if product_dep.nil?
  ...
  puts "[wire-rishi-settings] Added product dependency #{PKG_NAME} to rishi target"
else
  puts "[wire-rishi-settings] Product dependency #{PKG_NAME} already on rishi target"
end
...
unless already_linked
  ...
  puts "[wire-rishi-settings] Linked #{PKG_NAME} into rishi frameworks build phase"
else
  puts "[wire-rishi-settings] #{PKG_NAME} already in rishi frameworks build phase"
end

project.save
puts "[wire-rishi-settings] Saved #{PROJ_PATH}"
