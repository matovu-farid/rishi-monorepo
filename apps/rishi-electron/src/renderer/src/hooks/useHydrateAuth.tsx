        ...
        try {
          ...
          try {
            ...
            if (
              ...
              console.log(
                '[useHydrateAuth] persisted OAuth state is stale/terminal:',
                status.status
              )
  ...
  async function attemptCompleteAuth(state: string): Promise<void> {
    ...
    for (let attempt = 1; attempt <= MAX_AUTH_RETRIES; attempt++) {
      ...
      try {
        ...
        console.error(`[useHydrateAuth] auth attempt ${attempt}/${MAX_AUTH_RETRIES} failed:`, error)
        ...
        if (attempt < MAX_AUTH_RETRIES) {
          ...
          const delay = is409 ? BASE_RETRY_DELAY_MS * 3 : BASE_RETRY_DELAY_MS * attempt
